...
#include <cortex-flow/vm/NativeCallback.h>
#include <cortex-flow/FlowParser.h>
#include <cortex-flow/FlowLexer.h>
#include <cortex-flow/AST.h>
...
struct fntrace {
  ...
  fntrace(const char* msg) : msg_(msg) {
    ...
    CORTEX_DEBUG("FlowParser", 5, "%s", fmt);
    ++fnd;
  }
...
#define TRACE(level, msg...) CORTEX_DEBUG("FlowParser", (level), msg)
#else
